...
/**
 * Is it the agent, or would they have paid anyway?
 *
 * Moved here from the old standalone Evidence page: the answer to "is this
 * working" belongs beside the recovery figures it qualifies, not on a page a
 * merchant has to remember to visit separately.
 */
...
function CausesRateCard({ causes }: { causes: CausePerformance[] }) {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <p className="text-muted-foreground text-sm">
          Recovery rate by root cause, ordered by what is at stake. A cause
          that almost never comes back is a dunning problem, not a chasing one.
        </p>
      </CardHeader>
      <CardContent>
...
export function Overview({
  slug, initial, lift, spend, causes,
}: {
  slug: string;
  initial: Dashboard;
  /** Contacted-vs-held-back and its cost, over the same window as everything
   *  above - a snapshot from the server rather than kept live by the stream,
   *  since it changes slowly relative to a board row. */
  lift: Lift;
  spend: Spend;
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-4">
        <h2 className="text-lg font-semibold tracking-tight">Is it working?</h2>
